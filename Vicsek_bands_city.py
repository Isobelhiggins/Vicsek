...
# barriers = [(10, 20, 10, 20), # (x_start, x_end, y_start, y_end)
#             (10, 20, 30, 40),
#             (30, 40, 10, 20),
#             (30, 40, 30, 40)]
barriers = []
...
def random_barriers(barriers):
    for i in range(num_barriers):
        while True:
            ...
            x_start = np.random.randint(0, L - width)
            y_start = np.random.randint(0, L - height)
            x_end = x_start + width
...
def animate(frames):
    ...
    np.savez_compressed(f"pos_ang_arrays/bands_city/frame{frames}.npz", positions = np.array(positions, dtype = np.float16), angles = np.array(angles, dtype = np.float16))
    return qv,
...
# plt.tight_layout()
# plt.savefig("city_flux_17.png", dpi = 300)
# plt.show()
...
# plt.tight_layout()
# plt.savefig("city_stream_17.png", dpi = 300)
# plt.show()

# print(f"Number of Clusters: {num_clusters}")
# print(f"Average Particles per Cluster: {avg_cluster_particles}")